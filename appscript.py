...
class ClusteringPredictor(Predictor):
    ...
    def predict(self, user_data, content_type):
        ...
        return top_similar_games_euclidean
...
# Get user input (userId) from the user
user_id = st.text_input('Enter your user ID:')
if user_id:
    # Make prediction only if user ID is provided
    # 모델과 추론 스크립트 연결
    predictor = ClusteringPredictor(model=kmeans_model, scaler=scaler, pca=pca, features=features)

    # 추론 요청
    userId_data = {'userId': int(user_id)}  # 사용자 아이디 등 추론에 필요한 데이터
    response = predictor.predict(json.dumps(userId_data), content_type='application/json')

    # Display the recommendation
    st.subheader('Top 20 Recommended Games:')
    for item in response:
        st.write(f"Rank {item['Ranking']}: {item['Game']}")
